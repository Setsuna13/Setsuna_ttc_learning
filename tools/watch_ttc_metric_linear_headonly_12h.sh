...
EXP="${EXP:-ttc_metric_linear_headonly_full_v1}"
EXPECTED_EPOCH="${EXPECTED_EPOCH:-4}"
...
RUN_LOG="${RUN_LOG:-${EXP_DIR}/watch_12h_train.nohup.log}"
...
if mkdir "$LOCK_DIR" 2>/dev/null; then
  ...
  if [[ "$lock_pid" =~ ^[0-9]+$ ]] && kill -0 "$lock_pid" 2>/dev/null; then
    log "Watcher lock exists at ${LOCK_DIR}; pid=${lock_pid} is still running. Exiting."
    exit 0
  fi
  log "Removing stale watcher lock at ${LOCK_DIR}."
  rm -f "${LOCK_DIR}/pid"
  if rmdir "$LOCK_DIR" 2>/dev/null && mkdir "$LOCK_DIR" 2>/dev/null; then
    lock_acquired=true
  else
    log "Could not acquire watcher lock at ${LOCK_DIR}; exiting."
    exit 1
  fi
fi
